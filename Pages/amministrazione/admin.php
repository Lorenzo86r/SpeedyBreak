...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($tabella == 'SB_categoria') {
        ...
        if ($azione == 'add') {
            ...
            if (empty($password)) {
                $message = "<div class='alert alert-warning'>La password è obbligatoria per i nuovi utenti!</div>";
                $sql = null;
            } else {
                $hash = password_hash($password, PASSWORD_DEFAULT);
                $hash_escaped = $conn->real_escape_string($hash);
                $sql = "INSERT INTO SB_utente (username, email, ruolo, password_hash) VALUES ('$username', '$email', '$ruolo', '$hash_escaped')";
            }
        } else {
            // In modifica non si tocca mai la password
...
<body class="bg-light">
                ...
                <div class="modal-body" id="modalBody">
                    ...
                    <?php elseif ($tabella == 'SB_utente'): ?>
                        ...
                        <div id="campo_password">
                            ...
                            <input type="password" name="password" id="input_password" class="form-control mb-2">
                        </div>
    ...
    <script>
        ...
        function apriModalAggiungi() {
            ...
            // Mostra e rendi obbligatorio il campo password
            const campoPw = document.getElementById('campo_password');
            const pwField = document.getElementById('input_password');
            if (campoPw) campoPw.style.display = 'block';
        ...
        function apriModalModifica(data) {
            ...
            // Nascondi e svuota il campo password in modifica
            const campoPw = document.getElementById('campo_password');
            const pwField = document.getElementById('input_password');
            if (campoPw) campoPw.style.display = 'none';
            if (pwField) { pwField.required = false; pwField.value = ''; }
