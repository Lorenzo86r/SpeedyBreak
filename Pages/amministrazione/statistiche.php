...
$top_utenti = [];
while ($r = $res_utenti->fetch_assoc()) { $top_utenti[] = $r; }
...
$top_prodotti = [];
while ($r = $res_prod->fetch_assoc()) { $top_prodotti[] = $r; }
$stmt_top_prod->close();
...
<head>
    ...
    <style>
        .page-header { background: linear-gradient(135deg, rgba(8, 145, 178, 0.05), rgba(79, 70, 229, 0.05)); padding: 40px 20px; text-align: center; border-bottom: 1px solid var(--color-border); }
        ...
        .kpi-card { background: white; padding: 30px; border-radius: 16px; border: 1px solid var(--color-border); box-shadow: var(--shadow-sm); text-align: center; }
        .kpi-value { font-size: 48px; font-weight: 700; color: #10b981; line-height: 1; margin: 15px 0; }
        .kpi-label { font-size: 18px; color: var(--color-text-muted); font-weight: 500; text-transform: uppercase; letter-spacing: 1px; }
        
        .grid-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-top: 40px; }
        @media (max-width: 768px) { .grid-2 { grid-template-columns: 1fr; } }
        
        .stats-card { background: white; border-radius: 16px; border: 1px solid var(--color-border); box-shadow: var(--shadow-sm); overflow: hidden; }
        .stats-header { background: var(--color-background); padding: 20px; border-bottom: 1px solid var(--color-border); }
        .stats-header h3 { font-size: 18px; font-weight: 600; color: var(--color-text); }
        
        .stats-list { list-style: none; margin: 0; padding: 0; }
        .stats-item { display: flex; justify-content: space-between; align-items: center; padding: 15px 20px; border-bottom: 1px solid var(--color-border); }
        .stats-item:last-child { border-bottom: none; }
        .stats-rank { width: 30px; height: 30px; background: rgba(59, 130, 246, 0.1); color: #3b82f6; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 14px; margin-right: 15px; }
        .stats-name { flex: 1; font-weight: 500; }
        .stats-value { font-weight: 700; color: var(--color-text); background: rgba(0,0,0,0.05); padding: 4px 12px; border-radius: 99px; font-size: 14px; }
    </style>
</head>
<body>
    ...
    <main class="main-content">
        <div class="dashboard-container">
            ...
            <div class="grid-2">
                <div class="stats-card animate-fade-in" style="animation-delay: 0.1s; opacity: 0; animation-fill-mode: forwards;">
                    ...
                    <ul class="stats-list">
                        <?php if(empty($top_utenti)): ?>
                            <li class="stats-item"><span style="color: var(--color-text-muted);">Nessun dato disponibile</span></li>
                        <?php else: ?>
                            ...
                            <li class="stats-item">
                                <div style="display: flex; align-items: center; flex: 1;">
                                    ...
                                    <div class="stats-name">
                                        ...
                                        <div style="font-size: 12px; color: var(--color-text-muted); font-weight: 400;"><?= htmlspecialchars($u['email']) ?></div>
                                    </div>
                                </div>
                ...
                <div class="stats-card animate-fade-in" style="animation-delay: 0.2s; opacity: 0; animation-fill-mode: forwards;">
                    <div class="stats-header">
                    ...
                    <ul class="stats-list">
                        ...
                        <?php else: ?>
                            ...
                            <li class="stats-item">
                                <div style="display: flex; align-items: center; flex: 1;">
                                    <div class="stats-rank" style="background: rgba(16, 185, 129, 0.1); color: #10b981;"><?= $i + 1 ?></div>
